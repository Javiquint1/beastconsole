...
const allBlockIds: BlockId[] = ["google-ads", "meta-ads", "tiktok-ads", "linkedin-ads", "hubspot-crm", "salesforce-crm", "semrush-seo", "email", "free-ai"];
...
export function buildClientAccounts(database: PortalDatabase): ClientAccount[] {
  ...
  const adminAccounts = database.users
    ...
    .map((user) => ({
      ...
      enabledBlocks: ["google-ads", "meta-ads", "tiktok-ads", "linkedin-ads", "hubspot-crm", "salesforce-crm", "semrush-seo", "email", "free-ai"] as BlockId[],
      monthlyBudget: 0,
      leadGoal: 0
    }));
    ...
    client_app_access: clients.flatMap((client) =>
      client.role === "client"
        ? [
            ...
            { appId: "semrush_seo" as const, blockId: "semrush-seo" as const, trialEnabled: false }
          ].map((app) => ({
