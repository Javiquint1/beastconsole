...
export function requireTikTokClientId(request: NextRequest) {
  const clientId = request.headers.get("x-beast-client-id");
  if (!clientId) throw new Error("UNAUTHORIZED");
  return clientId;
}
export function requireTikTokAdmin(request: NextRequest) {
  if (request.headers.get("x-beast-role") !== "admin") throw new Error("FORBIDDEN");
}
export function tikTokApiError(error: unknown) {
  const message = error instanceof Error ? error.message : "TikTok Ads request failed.";
